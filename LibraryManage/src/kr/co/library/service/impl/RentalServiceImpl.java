...
public class RentalServiceImpl implements RentalService {
	...
	public String RentalPenaltyRevocation(UserManagement user) throws PenaltyUnendedException, PenaltyNotException {
		...
		System.out.println("RentalPenaltyRevocation - user = " + user);
		try {
			if(user.getPenaltyState()=='Y'){
				Calendar penaltyDay = Calendar.getInstance(); //패널티 계산을 위해 Calendar를 생성
				
				System.out.println("RentalPenaltyRevocation - rentalDao.selectRentalListByUserIdMaxEnd(session, user.getUserId() - " + rentalDao.selectRentalListByUserIdMaxEnd(session, user.getUserId()).get(0));
